            ...
            {movies.results?.length > 0 &&
                <div className="w-2/3 xl:w-3/4 mx-auto grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-16 justify-center items-center">
                    {movies.results.map(movie =>
                        <div key={movie.id} className="mx-auto rounded-md h-64 w-44 bg-slate-400 duration-500 shadow-slate-700 shadow-md cursor-pointer hover:shadow-lg hover:h-72 hover:w-48 flex flex-col movies-card">
